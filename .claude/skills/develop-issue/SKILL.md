...
GitHub issue を受け取り、その目的を達成する。任意の repo で使える汎用スキルであり、repo 固有の情報は本体にも LESSONS にも書かず、repo ノート (置き場は「学びと改善」節) に置く。ここに書くのはゴールと境界だけで、手順は状況で決める。repo の規約と会話中の指示が優先。
...
- issue に要約コメント (やったこと / 判断と理由 / 受け入れ条件との突合 / PR 一覧 / 未解決の懸念 / あれば再開の条件) を投稿済み。受け入れ条件は各項目ごとに達成・未達成を明示する。green 条件の実行結果や CI run へのリンクを根拠として添える。CI 未完了の時点で投稿した場合は、CI green 確認後にコメントを編集して CI run リンクを追記する
...
- 複数 PR に分割すると決めたら、実装着手前に issue へ計画を投稿する。項目ごとに green 条件つきの checklist にし、検証を通過した項目だけチェックする。区切りごとに更新し新着コメントも取り込む。それ以外は投稿せず、方針と理由は要約コメントの「判断と理由」に寄せる
- 実装前に green 条件 (走らせるコマンドと期待結果) を決める。実行する手段がなければ検証手段から作って repo に残す (失敗を再現するテスト、検証スクリプト)。それも無理な範囲は未検証と明記し、検証を可能にする環境整備を別 PR で提案する。ユーザ操作で再現する issue は、実際に起動して操作する検証も含める
...
- PR を作る前に fresh context の subagent に diff をレビューさせる。issue の受け入れ条件と green 条件 (計画を投稿したならそれも) を渡し、正確性と要件の gap だけ指摘させる
- worker の報告は主張として扱い、green 条件のコマンドは自分で再実行して確かめる。ユーザ・issue・PR body への報告は検証済みの事実だけ。PR body は後から読む人がレビューの正とする記述なので、commit を積んだら body の記述も head commit の実物と突き合わせ直す (古いまま残すと成果物と逆の主張になる)
...
探索やログ解析などコンテキストを食う作業は、規模によらず subagent に隔離して結論だけ受け取る。リスクの高い変更は観点の異なるレビューを複数走らせる。run が長引いたら、issue のコメントと push 済みの commit を引き継ぎ資料に fresh context へ交代する。モデルの使い分けと team の形は [references/model-notes.md](references/model-notes.md)。

## Guardrails

- merge はしない（review-and-merge ステージに委ねる）
- PR 作成後、`python3 .claude/scripts/check_protected_paths.py --diff origin/main` で保護パスへの該当を確認し、該当する場合は該当ファイルと理由を issue にコメントする。`hold` ラベルは付けない (保護パスは review が毎 run diff から機械判定して自分で付ける。先回りすると review はそれを「人間が止めた PR」として素通りし、一次レビューも PR への理由コメントもオーナーへの通知も無いまま人間待ちになる。実測 #396)
