...
- 着手前に issue へ計画を投稿する。単一 PR で設計判断を含まない変更では省略してよい (要約コメントで足りる)。分割する場合は項目ごとに green 条件つきの checklist にし、検証を通過した項目だけチェックする。区切りごとに更新し新着コメントも取り込む。人間が走行中に軌道修正できる接点であり、クラッシュ後の再開にも効く
