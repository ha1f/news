...
- 分割 (PR 単位・sub-plan) は [references/task-splitting.md](references/task-splitting.md) の基準で作る。issue や指示の曖昧さは orchestrator である自分が解消し、subagent には確定した仕様だけを渡す
...
subagent はこの会話を見られない。sub-plan は初見で end-to-end 実装できる自己完結文書として渡し、手順は細かく指定しない。リスクの高い変更では、観点の異なるレビュー subagent を複数走らせて検証を厚くする。モデルの使い分けと各モデルの癖は [references/model-notes.md](references/model-notes.md)。
...
走るたびに環境を良くする。issue の枠にとらわれず、ゴールと未来の run を見据える:

- 開始時に [LESSONS.md](LESSONS.md) を読み、run 中に得た教訓を追記する (書き方は同ファイル冒頭)
- run をやりにくくした環境の不足 (コーディングルールの欠落、古いライブラリ、検証の穴) を見つけたら、issue の範囲外でも別 PR / issue として提案する
