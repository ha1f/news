...
記事リストで読者に事前提示する情報。リンクを開く前に「読めるかどうか」が分かるようにする。

- **リンク先の言語**: 日本語
- **購読**: 無料
- **補足**: リンク先は外部サイト。ほぼ日本語
- **記事ドメイン**: b.hatena.ne.jp

## 利用条件

- **AI利用**: 可（規定なし）
- **商用利用（課金）**: 要許諾（商用は特別許諾制）
- **広告掲載での利用**: 要許諾（同上）
- **義務**: 規定なし
- **制約**: RSS 取得は30分毎程度まで（現行の日次取得は適合）。取得情報の第三者への再配布許諾は禁止
- **根拠**: https://developer.hatena.ne.jp/license （確認日 2026-08-28）

## 実装

[`feed_sources/hatena.py`](../../scripts/feed_sources/hatena.py)
