...
見出しだけでは伝わらない「この記事を開く理由」を1文で添える。**全記事に必ず付ける**。1件でも空くと、読者はその項目だけ見出しだけで開くかを判断することになり、流し読みのリズムが止まる（判断できずに保留され、結局読まれない）。

- 20〜40字程度。「読んだら何がわかるか」の観点で書く
- description がある場合は、そこから事実と論点を抽出して書く
- description が空・見出しとほぼ同内容の場合も省略しない。見出しの言い換えではなく、**手元で確認できる事実**（記事の種類・発信者・ソースでの扱われ方）から「開くと何が得られるか」を書く:

  | 手元にある事実 | 読みどころの書き方 |
  |---|---|
  | URL が発信元の公式ドメイン（プレスリリース等） | 「メーカー公式の発表。仕様と提供時期を原文で確認できる。」 |
  | URL が当事者・開発元のブログ | 「開発元による解説。設計上の選択を自ら説明している。」 |
  | ソートに使ったスコアが高い（議論が伸びている） | 「Hacker News で議論が伸びた1本。コメント欄も読みどころ。」 |
  | 一次論文・仕様書 | 「原論文。手法と検証条件を一次情報で確認できる。」 |

- 中身を推測して書かない。読んでいない記事の結論・数値・評価を書くのは禁止。確認できた事実だけで書く（GUARDRAILS.md のコンテンツの権利ガードレールにも沿う: 事実の提示にとどめ、原文の翻案をしない）
