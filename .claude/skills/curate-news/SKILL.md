...
- description が空・見出しとほぼ同内容の場合も省略しない。まず見出しと URL から実質的に書ける内容（発信元・記事の種類）がないか確認し、あればそれを書く。無いときだけ下表の型を使う。見出しの言い換えではなく、手元で確認できる事実（記事の種類・発信者・ソースでの扱われ方）から「開くと何が得られるか」を書く:

  | 手元にある事実 | 読みどころの書き方 |
  |---|---|
  | URL が発信元の公式ドメイン（プレスリリース等） | 「メーカー公式の発表リリースそのもの。原文で詳細を確認できる。」 |
  | URL が当事者・開発元のブログ | 「開発元による解説。設計上の選択を自ら説明している。」 |
  | Hacker News で URL が `news.ycombinator.com`（外部リンクが無い項目） | 「Hacker News で議論が伸びた1本。コメント欄も読みどころ。」 |
  | Hacker News で URL が外部記事（ソートに使ったスコアが高い） | 「Hacker News 経由で話題になった1本。」（HN の URL は外部記事に直接飛び、コメント欄には遷移しないため「コメント欄も読みどころ」とは書かない） |
  | 一次論文・仕様書 | 「原論文。手法と検証条件を一次情報で確認できる。」 |

- 読んでいない記事の結論・数値・評価は書かない。確認できた事実だけで書く（GUARDRAILS.md のコンテンツの権利ガードレールにも沿う: 事実の提示にとどめ、原文の翻案をしない）
