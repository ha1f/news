...
- **見出しと合わせて「誰が・何を」に答えられる状態にする**（#371）。主語を省かない（「株価は時間外で8%上昇」→「Accenture 株が時間外で8%上昇」）。その日はじめて出す固有名詞（企業・製品・モデル名）には、何者かを一言添える（「新型AI『Jev』」→「TypeSafe AI の新モデル『Jev』」）。見出し側で足りていればここで繰り返さない
  - **足すぶんは削る。** 字数の目安は変えない。詳細を落として輪郭を優先する（厚い要約は原文の翻案に近づく。GUARDRAILS.md のコンテンツの権利ガードレール）
