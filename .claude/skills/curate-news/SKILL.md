...
4. **興味カテゴリの検証**: 各候補が好みファイルの「興味・関心」のいずれかに該当するか確認する。いずれにも該当しない記事は除外する（ただし、テック業界に大きなインパクトを与える異分野のニュースは例外として残してよい）
5. 興味・関心との関連度、スコアから有力候補を選ぶ
...
多様性の担保:
- 同カテゴリからは2件までにする。読んでいて飽きないよう、興味カテゴリ間でジャンルを散らす
