...
- **誰が・何をしたのかを残す。** 短縮で最初に落ちるのが主語と「その固有名詞が何なのか」で、落ちるとその話題を初めて見る読者は輪郭を掴めない（#371）。削るのは修飾と細部で、主語ではない
...
- **見出しと合わせて「誰が・何を」に答えられる状態にする**（#371）。主語を省かない。その日はじめて出す固有名詞（企業・製品・モデル名）には、何者かを一言添える（「新型AI『Jev』」→「TypeSafe の新モデル『Jev』」）。見出し側で足りていればここで繰り返さない
  - **足すぶんは削る。** 字数の目安は変えず、細部を落として輪郭を優先する（厚い要約は原文の翻案に近づく。GUARDRAILS.md のコンテンツの権利ガードレール）。所有者の無い「株価が8%上昇」のような句は、**所有者を補うか、句ごと落とすかのどちらか**にする（宙に浮いたまま残さない）。英字の固有名詞は字数を食うので、当事者が複数いる回ほど細部を捨てる
...
- `meta` を訳しただけの1行にしない。媒体が書いた紹介文をなぞるのではなく、そこから事実を取り出して自分の言葉にする（GUARDRAILS.md のコンテンツの権利ガードレール）。TechCrunch のように `meta` が「誰が何を主張した」の1文で完結している媒体では直訳が最短経路になり、9/19・9/20 の突合で2件がほぼ直訳だった。`body` から別の事実（数値・当事者・時期）を1つ拾って組み立てる
- 取得できない・`thin: true` で `meta` も汎用文の場合は、**別の一次経路**を当たる（論文なら DOI から Crossref、GitHub なら raw、Apple の docs なら `developer.apple.com/tutorials/data/...json` など、そのページの内容を返す公開エンドポイント）
  - **本文が厚くても、初出の固有名詞が何者かを本文から確認できないときは同じように当たる**（製品・モデルなら公式ドキュメント）。記事は読者がその名前を知っている前提で書かれていることが多く、「取得は成功したが何者かだけ分からない」は普通に起きる。ここで諦めると、名前を説明できないまま出すか項目を落とすかの二択になる（#371 の1件目がこれ）
