...
PR を作成したら `python3 .claude/scripts/check_protected_paths.py --diff origin/main` で保護パスへの該当を確認し、該当する場合は該当ファイルと理由を issue にコメントする。`hold` ラベルは付けない (保護パスは review が毎 run diff から機械判定して自分で付ける。先回りすると review はそれを「人間が止めた PR」として素通りし、一次レビューも PR への理由コメントもオーナーへの通知も無いまま人間待ちになる。実測 #396)
