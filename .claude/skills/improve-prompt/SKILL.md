...
繰り返し使われるプロンプト（スキル、システムプロンプト）への構造的な変更には、[references/eval-guide.md](references/eval-guide.md) の検証を提案する。手段は目的で選ぶ（必要なら両方）: 改善効果の判定は新旧を fresh context の subagent で並列実行する blind 比較、曖昧さ・矛盾の検出は机上シミュレーション。1 回の印象より複数 trial。効果が確認できたら、使ったシナリオを対象側の evals/ に残して回帰資産にする。軽微な変更（数行の rule 修正等）には提案しない。
