...
- status issue に開始と終了の各1コメント。1行目は check_state.py が機械判定する JSON（キー名・値とも厳密一致が必要）:
  ...
  - 終了: `{"stage": "develop", "phase": "end", "ok": true, "summary": "#26 実装 → PR #27。open issue 9/10", "reflect": "LESSONS.md 更新1件"}`
    - `summary` の末尾に、手順1の `select_issues.py` が出した `open_issues` と `config.open_issue_cap` を `open issue {open_issues}/{open_issue_cap}` の形でそのまま貼る（翌日の PdM が cap 超過を run をまたいで拾えるようにする #394）。**何を数えて何を除くかはスクリプトが持っているので、ここでも終了コメントでも書かない。** run の途中で issue を起票したら、貼る前に `select_issues.py` を実行し直して起票後の値にする
