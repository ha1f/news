...
続けて、デフォルトプロファイルの各項目を**予備知識のない読者として上から読み、「誰が・何を」に口で答える**（#371）。答えられない項目は、curate-news の「各記事の見出し」「読みどころ」節に従って主語か固有名詞の説明を補う（字数は増やさず、詳細を削って入れ替える）。この失敗モードは正規表現では十分な精度が出なかったため、検査スクリプトでなく読み手の確認に置いている（#371）。
