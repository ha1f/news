...
投稿ファイルを作ったら、各項目に読みどころが付いているか確認する。1件でも欠けていれば、curate-news の「読みどころ」節の書き方に従って補ってから次へ進む。引数なしで実行すると内部で当日 JST の日付を再計算してしまい、JST 日跨ぎで対象ファイルが見つからず空振り exit 1 になる（ステップ 1 で確定した日付を使い回す方針に反する）ため、確定済みの `{YYYY-MM-DD}` を明示して渡す:
