...
対象プロファイルが1件なら Skill ツールで `curate-news` を直接実行する（デフォルトは引数なし、名前付きは `--profile {name}` を `args` に渡す）。

対象プロファイルが複数件なら、1件は自分で直接実行し、残りの各プロファイルは Agent ツールで1エージェントずつ並列実行するとよい（各エージェントに `.claude/skills/curate-news/SKILL.md` を読んで手順通り実行させ、`--profile {name}` が必要なスクリプトに付与するよう指示する。`fetch_feeds.py` の `--show-cache-summary` には `--profile` を付けない）。キャッシュはソース・カテゴリ単位のファイルに分かれているため、複数プロファイルが同時に読み書きしても衝突しない。逐次実行よりはるかに短時間で終わる。
...
複数プロファイルを並列実行する場合、Reddit 系ソース（reddit-swift 等）は全体で共有のレート制限にかかりやすく、単一プロファイルなら成功する取得でも 429 で失敗することがある。1エージェントが失敗しても他のソースで代替できていれば、根本原因を追わずそのままスキップしてよい。
