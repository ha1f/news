...
フロントマターの `title` は記事ページの見出し・一覧の見出しリンク・共有時のタイトル・OGP・RSS の項目名すべてに使われる。**日付だけの文字列にしない**（#341）。リード文と記事一覧を読み、その日いちばん大きい話題を凝縮した見出しを日本語で作る（目安15〜30字。テンプレートに当てはめず、実際の内容から判断する）。日付は一覧の日付表示・記事ページの `<time>` など別要素で毎回表示されるため、`title` に日付を含めない（含めると隣接して二重表示になる）。本文はリード文から始める（curate-news 出力の先頭にある `## ニュース (...)` 見出し行はタイトルと重複するので含めない）。
