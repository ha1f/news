...
投稿ファイルを作ったら、各項目の読みどころを確認する。「読みどころなし」なら補い、「出どころの紹介だけ」なら curate-news の「読みどころ」節に従って記事ページから事実を取り直す（取れなければその項目を差し替える）。どちらも解消してから次へ進む。引数なしで実行すると内部で当日 JST の日付を再計算してしまい、JST 日跨ぎで対象ファイルが見つからず空振り exit 1 になる（ステップ 1 で確定した日付を使い回す方針に反する）ため、確定済みの `{YYYY-MM-DD}` を明示して渡す:
