...
キュレーションはコンテキストを大きく消費するので、オーケストレータ自身は実行せず、対象プロファイルそれぞれについて Agent ツールで fresh context の subagent を並列起動する（1件のみの場合も同様）。各 subagent への指示:

> `curate-news` スキルを Skill ツールで実行する（デフォルトプロファイルなら引数なし、名前付きなら `--profile {name}`）。完了したら、curate-news のステップ6「結果の保存」で**自分が書き出した**出力先のフルパスと、そのファイル名のステム部分（例: `2026-03-18-23cfb1cf`）を報告する。パスが不確かなら `python3 .claude/skills/curate-news/scripts/preference_hash.py [--profile {name}]` でサフィックスを取り直して組み立てる。`ls` で列挙しないこと（同じ日付のファイルが他プロファイルの分も並ぶため、自分の出力を判別できない）。

各 subagent は担当プロファイルを指定して起動するので、**プロファイル名と出力ファイルの対応はディスパッチした時点で決まっている**。subagent の報告からステムと出力ファイルのパスを記録し、パスから出力ファイルの内容を Read で読み込む。

並列実行の安全性は `fetch_feeds.py` 側で担保している（キャッシュキー単位の排他ロックと atomic write。好みが被って同じソース・カテゴリを複数 subagent が取りにいっても、取得は1回に収束し、キャッシュが壊れない）。Reddit の10リクエスト/分（`references/sources/reddit.md`）に当たったときの対処は `curate-news/SKILL.md` の「取得失敗時のfallback」にあり、各 subagent がそれ自身読んで従う。
