...
対象プロファイルは互いに独立している（ソース・カテゴリ単位でキャッシュファイルが分かれるため、並列実行してもキャッシュの読み書きは衝突しない。実測: 5プロファイル並列で約8分）。コンテキストを大きく消費する作業なので、オーケストレータ自身はキュレーションを実行せず、対象プロファイルそれぞれについて Agent ツールで fresh context の subagent を並列起動する（1件のみの場合も同様）。各 subagent への指示:

> `curate-news` スキルを実行する（デフォルトプロファイルなら引数なし、名前付きなら `--profile {name}`）。完了したら `ls .claude/skills/curate-news/output/{YYYY-MM-DD}*.md` で出力ファイルを特定し、そのフルパスとファイル名のステム部分（例: `2026-03-18-23cfb1cf`）を報告する。

各 subagent には担当プロファイルを指定して起動するため、**プロファイル名と出力ファイルの対応はディスパッチした時点で決まっている**（複数の出力が同時に出そろっても、どれがどのプロファイルかを後から ls で推測する必要がない）。subagent の報告からステムと出力ファイルのパスを記録し、パスから出力ファイルの内容を Read で読み込む。

Reddit は非認証で10リクエスト/分の制限がある（`references/sources/reddit.md`）。並列実行では複数の subagent が同時にアクセスしうるため 429 が起きやすくなるが、対処は `curate-news/SKILL.md` の「取得失敗時のfallback」に既にあり、各 subagent がそれ自身読んで従うため、ここに追加の指示は要らない。
...
名前付きプロファイル（タイトルにプロファイルの日本語表示名を付加し、同日の他プロファイル記事と区別する）。`{表示名}` は `profiles/{name}.md` の `title` フロントマターから取得する（例: engineer → ソフトウェアエンジニア）:
```yaml
---
layout: post
title: "{YYYY}年{M}月{D}日（{表示名}）"
date: {YYYY-MM-DD}
profile: {name}
tags: [{該当トピックをカンマ区切り}]
---
...
投稿ファイルを作ったら、各項目に読みどころとソース表記が付いているか確認する。1件でも落ちていれば、curate-news の「読みどころ」「ソース名と読者向け属性」節に従って補ってから次へ進む。引数なしで実行すると内部で当日 JST の日付を再計算してしまい、JST 日跨ぎで対象ファイルが見つからず空振り exit 1 になる（ステップ 1 で確定した日付を使い回す方針に反する）ため、確定済みの `{YYYY-MM-DD}` を明示して渡す:
