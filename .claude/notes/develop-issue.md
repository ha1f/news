...
- マージすると `pages.yml` の main ビルドが**数秒後に現れ、1分前後で completed** になる（実測 2026-09-22、直近のマージ済み PR 12件: run の出現がマージ後 2〜19 秒、completed まで 47〜77 秒、いずれも success）。run が現れるのを待つループで `?head_sha=` を使うなら **完全な40桁 SHA** を渡す。短縮 SHA は HTTP 200 / `total_count: 0` を返すだけなので永久に待つ（実測: 7桁・12桁とも 0件、40桁で1件）
  - **待ち方**: MCP にブロッキング待機は無い（`actions_get`/`pull_request_read(get_check_runs)` は都度呼ぶだけで `gh run watch` 相当が無い）。`ScheduleWakeup` は `/loop` の動的モード専用で、それ以外の文脈で呼ぶと `prompt` 必須のエラーか、出力なしの空ターンになる（実測 2026-09-23）。単発の `sleep N` も harness にブロックされる。`Monitor` に `curl -H "Authorization: token $GITHUB_TOKEN"` で対象 run/check を数秒間隔でポーリングし、終端状態（`completed`/`success`/`failure` 等）になったら1行 echo して抜けるループを渡すのが安定する（`$GITHUB_TOKEN` は素の REST 読み取りと同じ proxy 認証経路でそのまま使える）
...
- `Write` で保存したファイルの末尾に、まれに `</content>` や `</invoke>` などツール呼び出し構文そのものに見える文字列が紛れ込むことがある（実測 2026-09-23: `_posts/*.md` を5件 `Write` で作成し、5件とも末尾に混入。単独呼び出し・並列呼び出しの両方で発生）。`check_article_notes.py` / `check_source_hints.py` はどちらもこの余分な行を検査対象にしないため素通りする。`_posts/` など公開物を `Write` した直後は `tail -c 200 <file> | cat -A` で生バイトを確認し、混入していれば `Edit` で取り除いてから次に進む
