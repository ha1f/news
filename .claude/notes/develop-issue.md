...
- **ループが止まった原因は Claude Code Remote の MCP で辿れる。** `list_triggers` の `last_run.status` で失敗した run を特定し、その `session_id` を `get_session` に渡すと `post_turn_summary.status_detail` と `rate_limit_info` に理由が出る（MCP ツール名は `mcp__Claude_Code_Remote__*`）。アカウントの**週次利用上限の枯渇は「fire から6〜7秒で FAILED」**という形で現れる（実測 2026-09-25: 全ステージが `You've hit your weekly limit · resets 9pm (UTC)` / `rateLimitType: seven_day` で起動直後に弾かれ、約42時間ループが停止して当日の配信が丸ごと欠落した。#421）。trigger は `enabled: true` のまま・status issue にもレコードが残らないので、一覧を眺めるだけでは止まったことに気づけない
