...
- **ループによる引き取り** — Renovate が自動マージしない更新（major・Renovate 設定の移行・自動マージが止まった PR）は、日次ループの review ステージが changelog と CI を確認してマージまたは close する（[.claude/GUARDRAILS.md](.claude/GUARDRAILS.md) の信頼の軸: この repo に書き込める名義なら人間・bot を問わず対象）。人間の手は要らない。レビューで踏まえる Renovate の挙動:
